...
public class LongestPathInMatrix {
    ...
    private int dfs(int i, int j, int[][] matrix, int[][] dp) {
        ...
        if(i > 0 && isFeasible(matrix, i, j, i-1, j))
            one = 1 + dfs(i-1, j, matrix, dp);

        if(i+1 < matrix.length && isFeasible(matrix, i, j, i+1, j))
            two = 1 + dfs(i+1, j, matrix, dp);

        if(j > 0 && isFeasible(matrix, i, j, i, j-1))
            three = 1 + dfs(i, j-1, matrix, dp);

        if(j+1 < matrix[0].length && isFeasible(matrix, i, j, i, j+1))
    ...
    private boolean isFeasible(int[][] matrix, int i, int j, int ni, int nj) {
        return i >= 0
                && j >= 0
                && i<matrix.length
                && j<matrix[0].length
